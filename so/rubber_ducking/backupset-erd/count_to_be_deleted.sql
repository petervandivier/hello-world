...
declare @oldest_date datetime = dateadd(day,-7,getdate()); -- server time
  ...
  select count(*) as count_backupfile from msdb.dbo.backupfile
  WHERE backup_set_id IN (SELECT backup_set_id
                          FROM @backup_set_id)
UNION ALL 

  select count(*) as count_backupfilegroup from msdb.dbo.backupfilegroup
  WHERE backup_set_id IN (SELECT backup_set_id
                          FROM @backup_set_id)
UNION ALL 

  select count(*) as count_restorefile from msdb.dbo.restorefile
  WHERE restore_history_id IN (SELECT restore_history_id
                               FROM @restore_history_id)
UNION ALL 

  select count(*) as count_restorefilegroup from msdb.dbo.restorefilegroup
  WHERE restore_history_id IN (SELECT restore_history_id
                               FROM @restore_history_id)
UNION ALL 

  select count(*) as count_restorehistory from msdb.dbo.restorehistory
  WHERE restore_history_id IN (SELECT restore_history_id
                               FROM @restore_history_id)
UNION ALL 

  select count(*) as count_backupset from msdb.dbo.backupset
  WHERE backup_set_id IN (SELECT backup_set_id
                          FROM @backup_set_id)
UNION ALL 

  select count(*) as count_backupmediafamily
...
UNION ALL 
  ...
  select count(*) as count_backupmediaset
