...
@_msparam_0 = N'backupmediaset',
@_msparam_1 = N'dbo';
go
-- batch 3
declare
    @find_referencing_objects int;
...
create unique clustered index i1 on #t1 ( object_name, object_schema, object_db, object_svr, object_type, relative_name, relative_schema, relative_type
)
    with ignore_dup_key;
...
where
    tt.type_table_object_id = #t1.object_id and
    object_type = @udtt;
...
begin
    ...
    begin
        ...
        where @iter_no = t.rank and
              ( t.object_type = @uddt or
                t.object_type = @udt
        ) and
              t.object_svr is null and
              t.object_db = db_name();
        ...
        select
            ...
            @iter_no + 1
        from #t1 as t
        join sys.objects as o on o.parent_object_id = t.object_id and
                                 o.type = 'TR'
        where @iter_no = t.rank and
              ( t.object_type = @u or
                t.object_type = @v
        ...
        join sys.objects as o on o.object_id = clmns.object_id and
                                 0 = isnull(o.parent_object_id, 0)
        where @iter_no = t.rank and
              t.object_type = @def and
              t.object_svr is null and
              t.object_db = db_name();
        set @rows = @rows + @@rowcount;
        ...
        join sys.objects as o on o.object_id = t.object_id and
                                 0 = isnull(o.parent_object_id, 0)
        where @iter_no = t.rank and
              t.object_type = @def and
              t.object_svr is null and
        ...
        select
            ...
            @iter_no + 1
        from #t1 as t
        join sys.assembly_modules as am on am.assembly_id = t.object_id and
                                           am.assembly_id >= 65536
        join sys.objects as o on am.object_id = o.object_id
        where @iter_no = t.rank and
              t.object_type = @assm and
              t.object_svr is null and
              t.object_db = db_name();
        set @rows = @rows + @@rowcount;
        ...
        select
            ...
            @iter_no + 1
        from #t1 as t
        join sys.assembly_types as at on at.assembly_id = t.object_id and
                                         at.is_user_defined = 1
        where @iter_no = t.rank and
              t.object_type = @assm and
              t.object_svr is null and
              t.object_db = db_name();
        ...
        select
            ...
            @iter_no + 1
        from #t1 as t
        join sys.assembly_references as ar on ar.referenced_assembly_id = t.object_id and
                                              ar.referenced_assembly_id >= 65536
        ...
        join sys.objects as o on o.object_id = p.object_id and
                                 o.type = 'AF'
        where @iter_no = t.rank and
              t.object_type in ( @udt, @uddt, @udtt ) and
        t.object_svr is null and
        t.object_db = db_name();
        ...
        where @iter_no = t.rank and
              t.object_type in ( @sp, @udf, @tr ) and
        t.object_svr is null and
        t.object_db = db_name();
        set @rows = @rows + @@rowcount;
        ...
        for select distinct
            ...
            where @iter_no = t.rank and
                  t.object_svr is null and
                  t.object_db = db_name() and
                  t.object_type not in ( @part_sch, @assm, @tr, @ddltr );
        open name_cursor;
        declare
            @objid int;
        declare
        ...
        declare
            @objecttype sysname;
        fetch next from name_cursor into
            @objid,
            @objname,
            @objschema,
            @objtype;
        ...
        begin
            set @fullname = case
                                when @objschema is null then quotename(@objname)
                                else quotename(@objschema)+'.'+quotename(@objname)
                            end;
            set @objecttype = case
                                  when @objtype in(@uddt, @udt, @udtt) then 'TYPE'
        ...
        where
            o.object_id = #t2.object_id and
            ( #t2.object_type = @obj or
        ...
        select
            ...
            rank
        from #t2
        where @iter_no + 1 = rank and
              #t2.object_id != #t2.relative_id;
        set @rows = @rows + @@rowcount;
    ...
    begin
        ...
        join sys.objects as o on o.object_id = d.referencing_id and
                                 o.type = 'C'
        join sys.sql_expression_dependencies as dep on dep.referencing_id = d.referencing_id and
                                                       dep.referenced_id != t.object_id
        where @iter_no = t.rank and
              t.object_svr is null and
              t.object_db = db_name() and
              t.object_type = @u;
        ...
        for select distinct
            ...
            from #t2 as t
            where t.rank = @iter_no + 1 and
                  t.object_id is null and
                  t.object_svr is null;
        open db_cursor;
        declare
            @dbname sysname;
        declare
        ...
        begin
            ...
            set @query = 'update #t2 set object_db = N'+@quote_quoted_dbname+', object_id = obj.object_id, object_type = 
                            case when obj.type = ''U'' then '+cast(@u as nvarchar(8))+' when obj.type = ''V'' then '+cast(@v as nvarchar(8))+' when obj.type = ''TR'' then '+cast(@tr as nvarchar(8))+
            ' when obj.type in ( ''P'', ''RF'', ''PC'' ) then '+cast(@sp as nvarchar(8))+' when obj.type in ( ''AF'' ) then '+cast(@uda as nvarchar(8))+
            ' when obj.type in ( ''TF'', ''FN'', ''IF'', ''FS'', ''FT'' ) then '+cast(@udf as nvarchar(8))+' when obj.type = ''D'' then '+cast(@def as nvarchar(8))+' when obj.type = ''SN'' then '+
            cast(@synonym as nvarchar(8))+' when obj.type = ''SO'' then '+cast(@sequence as nvarchar(8))+' else '+cast(@unknown as nvarchar(8))+' end
                from '+@bracket_quoted_dbname+'.sys.objects as obj 
                join '+@bracket_quoted_dbname+
            ...
            exec (@query);
            -- @type
            set @query = 'update #t2 set object_db = N'+@quote_quoted_dbname+', object_id = t.user_type_id, object_type = case when t.is_assembly_type = 1 then '+cast(@udt as nvarchar(8))+
            ' when t.is_table_type = 1 then '+cast(@udtt as nvarchar(8))+' else '+cast(@uddt as nvarchar(8))+' end
            ...
            -- @obj
            set @query = 'update #t2 set object_db = N'+@quote_quoted_dbname+
            ', object_id = obj.object_id, object_schema = SCHEMA_NAME(obj.schema_id), object_type = 
                            case when obj.type = ''U'' then '+cast(@u as nvarchar(8))+' when obj.type = ''V'' then '+cast(@v as nvarchar(8))+' when obj.type = ''TR'' then '+cast(@tr as nvarchar(8))+
            ' when obj.type in ( ''P'', ''RF'', ''PC'' ) then '+cast(@sp as nvarchar(8))+' when obj.type in ( ''AF'' ) then '+cast(@uda as nvarchar(8))+
            ' when obj.type in ( ''TF'', ''FN'', ''IF'', ''FS'', ''FT'' ) then '+cast(@udf as nvarchar(8))+' when obj.type = ''D'' then '+cast(@def as nvarchar(8))+' when obj.type = ''SN'' then '+
            cast(@synonym as nvarchar(8))+' when obj.type = ''SO'' then '+cast(@sequence as nvarchar(8))+' else '+cast(@unknown as nvarchar(8))+' end
                from '+@bracket_quoted_dbname+
            '.sys.objects as obj 
                where obj.name = #t2.object_name collate database_default
            ...
            -- @type
            set @query = 'update #t2 set object_db = N'+@quote_quoted_dbname+
            ', object_id = t.user_type_id, object_schema = SCHEMA_NAME(t.schema_id), object_type = case when t.is_assembly_type = 1 then '+cast(@udt as nvarchar(8))+' when t.is_table_type = 1 then '+
            cast(@udtt as nvarchar(8))+' else '+cast(@uddt as nvarchar(8))+' end
                from '+@bracket_quoted_dbname+
            '.sys.types as t
                where t.name = #t2.object_name collate database_default
            ...
            set @query = 'update #t2 set object_db = N'+@quote_quoted_dbname+', object_id = p.function_id
                ...
                and #t2.object_type = '+cast(@part_func as nvarchar(8))+' and (#t2.object_db IS NULL or #t2.object_db = N'+@quote_quoted_dbname+')
                and #t2.rank = ('+cast(@iter_no as nvarchar(8))+'+1) and #t2.object_id IS NULL and #t2.object_svr IS NULL';
            exec (@query);
            ...
            where
                o.name = #t2.object_name collate database_default and
                sch.name = #t2.object_schema collate database_default and
                o.type in ( 'P', 'RF', 'PC' ) and
                #t2.object_id is null and
                #t2.object_name like 'sp/_%' escape '/' and
                #t2.object_db is null and
                #t2.object_svr is null;
            ...
            where
                o.name = #t2.object_name collate database_default and
                schema_name(o.schema_id) = 'dbo' collate database_default and
                o.type in ( 'P', 'RF', 'PC' ) and
                #t2.object_schema is null and
                #t2.object_id is null and
                #t2.object_name like 'sp/_%' escape '/' and
                #t2.object_db is null and
                #t2.object_svr is null;
        ...
        join sys.types as tp on tp.user_type_id = col.user_type_id and
                                tp.schema_id != 4
        where @iter_no = t.rank and
              t.object_type = @u and
              t.object_svr is null and
        ...
        join sys.objects as o on o.object_id = clmns.default_object_id and
                                 0 = isnull(o.parent_object_id, 0)
        where @iter_no = t.rank and
              t.object_type = @u and
              t.object_svr is null and
        ...
        join sys.objects as o on o.object_id = tp.default_object_id and
                                 0 = isnull(o.parent_object_id, 0)
        where @iter_no = t.rank and
              t.object_type = @uddt and
              t.object_svr is null and
        ...
        from #t1 as t
        join sys.columns as clmns on clmns.object_id = t.object_id
        join sys.objects as o on o.object_id = clmns.rule_object_id and
                                 0 = isnull(o.parent_object_id, 0)
        where @iter_no = t.rank and
              t.relative_type = @u and
              t.object_svr is null and
              t.object_db = db_name();
        set @rows = @rows + @@rowcount;
        ...
        from #t1 as t
        join sys.types as tp on tp.user_type_id = t.object_id
        join sys.objects as o on o.object_id = tp.rule_object_id and
                                 0 = isnull(o.parent_object_id, 0)
        where @iter_no = t.rank and
              t.relative_type = @uddt and
              t.object_svr is null and
              t.object_db = db_name();
        set @rows = @rows + @@rowcount;
        ...
        join sys.xml_schema_collections as x on x.xml_collection_id = c.xml_collection_id and
                                                x.schema_id != 4
        where @iter_no = t.rank and
              t.object_type = @u and
              t.object_svr is null and
        ...
        select
            ...
            @iter_no + 1
        from #t1 as t
        join sys.assembly_references as ar on ar.assembly_id = t.object_id and
                                              ar.referenced_assembly_id >= 65536
        ...
        select
            ...
            @iter_no + 1
        from #t1 as t
        join sys.assembly_types as at on at.user_type_id = t.object_id and
                                         at.is_user_defined = 1
        join sys.assemblies as asm on asm.assembly_id = at.assembly_id
        where @iter_no = t.rank and
              t.object_type = @udt and
              t.object_svr is null and
              t.object_db = db_name();
        set @rows = @rows + @@rowcount;
        ...
        select
            ...
            @iter_no + 1
        from #t1 as t
        join sys.assembly_modules as am on am.object_id = t.object_id and
                                           am.assembly_id >= 65536
        join sys.assemblies as asm on asm.assembly_id = am.assembly_id
        where @iter_no = t.rank and
              t.object_type in ( @udf, @sp, @uda, @tr ) and
        t.object_svr is null and
        t.object_db = db_name();
        set @rows = @rows + @@rowcount;
        ...
        join sys.objects as o on o.object_id = object_id(s.base_object_name) and
                                 o.type in('U', 'V', 'P', 'RF', 'PC', 'AF', 'TF', 'FN', 'IF', 'FS', 'FT')
        where @iter_no = t.rank and
              t.object_type = @synonym and
              t.object_svr is null and
              t.object_db = db_name();
        set @rows = @rows + @@rowcount;
        ...
        join sys.types as tp on tp.user_type_id = s.user_type_id and
                                tp.schema_id != 4
        where @iter_no = t.rank and
              t.object_type = @sequence and
              t.object_svr is null and
              t.object_db = db_name();
        set @rows = @rows + @@rowcount;
...
(
    select
        count(*)
    from #t1 t
    where t.relative_id = #t1.object_id and
          t.object_id != t.relative_id
);
...
for select distinct
        isnull(object_db, db_name())
    from #t1 as t
    where t.object_id is not null and
          t.object_svr is null;
open db_cursor;
fetch next from db_cursor into
    @dbname;
...
begin
    ...
    set @query = 'update #t1 set #t1.object_name = o.name,#t1.object_schema = sch.name from #t1  inner join '+quotename(@dbname)+'.sys.objects as o on #t1.object_id = o.object_id inner join '+
    quotename(@dbname)+'.sys.schemas as sch on sch.schema_id = o.schema_id  where o.name = #t1.object_name collate '+@collation+' and sch.name = #t1.object_schema collate '+@collation;
    exec (@query);
...
from #t1 as t
left join sys.objects as o on t.object_type = @tr and
                              o.object_id = t.object_id or
                              t.relative_type = @tr and
                              o.object_id = t.relative_id
